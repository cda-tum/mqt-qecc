...
def heuristic_prep_circuit_constrained(
    ...
    for i in range(rank):
        ...
        if pivot is None:
            continue
        check = checks[i]
        tree = rx.steiner_tree(coupling_map, support(check), lambda _: 1)
    ...
    circ = _build_state_prep_circuit_from_back(cnots, checks, zero_state)  # TODO: mapping
