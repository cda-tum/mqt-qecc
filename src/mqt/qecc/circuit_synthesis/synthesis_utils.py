...
def gaussian_elimination_min_column_ops(
    ...
    Args:
        ...
        coupling_map: coupling map of a device. If provided, only eliminations between connected columns are allowed.

    returns:
        The reduced matrix and a list of the elimination steps taken. The elimination steps are represented as tuples of the form (i, j) where i is the column being eliminated with and j is the column being eliminated.
        If a coupling map is provided, a dictionary mapping columns to device qubits is also returned.
    """
    s = z3.Solver()

    if coupling_map is not None:
        matrix, mapping = _create_mapping(matrix, coupling_map, s)
    ...
    if coupling_map is not None:
        s.add(_permute_matrix(matrix, mapping, columns[0, :, :]))
    ...
    if s.check() == z3.sat:
        ...
        if coupling_map is not None:
            column_mapping = {i: j for i in range(len(mapping)) for j in range(len(mapping[0])) if m[mapping[i][j]]}
            return reduced, eliminations, column_mapping
...
def _column_addition_constraint(
    columns: npt.NDArray[z3.BoolRef | bool], col_add_vars: npt.NDArray[z3.BoolRef], coupling_map: rx.PyGraph
) -> z3.BoolRef:
...
def optimal_elimination(
    ...
    coupling_map: rx.PyGraph = None,
) -> tuple[npt.NDArray[np.int8], list[tuple[int, int]], dict[int, int] | None] | None:
    ...
    if coupling_map is None:
        reduced_and_eliminations, curr_param = res
        if reduced_and_eliminations is None:
            return None
        reduced, eliminations = reduced_and_eliminations
    else:
        reduced_and_mapping, curr_param = res
        if reduced_and_mapping is None:
            return None
...
def support(check: npt.NDArray[np.int8]) -> npt.NDArray[np.int8]:
