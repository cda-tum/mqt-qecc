...
def gen_pcm_and_logical(distance):
    ...
    parity_check_matrix = np.zeros(
        (ancilla_qubit_count, qubit_count), dtype=bool)
    ...
    print(qubit_count, ancilla_qubit_count)
    return (parity_check_matrix, logical_operator)


def add_checks_one_round(pcm, circuit, detectors, error_probability):
    for check in pcm:
        if error_probability == 0:
            mpp_X_instruction = f"MPP "
            mpp_Z_instruction = f"MPP "
        else:
    ...
    if detectors is True:
        for q in range(len(pcm)):
            circuit.append(
                ...
                "DETECTOR", [stim.target_rec(-1*q-1), stim.target_rec(-1*q-len(pcm)-1)])
    return circuit


def gen_stim_circuit_memory_experiment(pcm, logical_operator, distance, error_probability):
