...
def quasi_cyclic_check_matrices(n: int) -> tuple[np.array, np.array]:
    ...
    if n == 72:
        ...
        raise ValueError(f'No quasi-cyclic code with n = {n}.')
...
def _cyclic_matrix_sum(x_terms, y_terms, l, m):
    ...
    a = np.zeros(x.shape)
    for pow in x_terms:
        a = a + np.linalg.matrix_power(x, pow)
    for pow in y_terms:
        a = a + np.linalg.matrix_power(y, pow)

    return a


def _cyclic_code_check_matrix(m: int, l: int, a_terms: tuple[list, list], b_terms: tuple[list, list]) -> tuple[np.array, np.array]:
